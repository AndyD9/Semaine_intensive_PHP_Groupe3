...
$requestPix = "SELECT * FROM pictures WHERE iduser =".$_SESSION['iduser']." ORDER BY idpictures DESC;";
...
<body>
  ...
  <?php while(false !== $row = $stmt->fetch(PDO::FETCH_ASSOC)):?>
    <div class="img-container">
      <div class="img-150-300">
        <img src="<?=$row['folder']?>" alt="pix">
        <div class="imghover">
          <i id="<?=$row['idpictures']?>" class="far fa-times-circle pictureDeleteBtn" style="font-size: 6vh;"></i>
        </div>
      </div>
    </div>
  <?php endwhile;?>
  ...
  <div class="profil-overlay">
    ...
    <script>
    let uploadBtn = document.querySelector('.header-upload');
    let uploadOverlay = document.querySelector('.upload-overlay');
    let closeBtn = document.querySelector('.upload-close-overlay');
    let profilOverlay = document.querySelector('.profil-overlay');
    let profileCloseOverlay = document.querySelector('.profile-close-overlay');
    let profilePictureBtn = document.querySelector('.header-profilePicture');
    let pictureDeleteBtn = document.querySelectorAll('.pictureDeleteBtn');
    ...
    for (let i = 0; i < pictureDeleteBtn.length; i++) {
      pictureDeleteBtn[i].addEventListener('click', function() {
        let deletePicture = pictureDeleteBtn[i].id;
        window.location.href = "doDelete.php?deletePicture=" + deletePicture;
      });
    }
    </script>
  </body>
